	...
	public function get_operators_for_rule( $rule ) {
		...
		if ( in_array( $rule, $numeric_options, true ) ) {
			if ( $this->rule_supports_multi_select( $rule ) ) {
				return [ 'exists', 'not_exists', 'equals', 'greater_than', 'less_than', 'includes_any', 'includes_all', 'excludes_any', 'excludes_all' ];
			} else {
				return [ 'exists', 'not_exists', 'equals', 'greater_than', 'less_than' ];
			}
		}
